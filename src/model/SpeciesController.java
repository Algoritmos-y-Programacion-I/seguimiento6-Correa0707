...
public class SpeciesController {
	...
	public SpeciesController() {
		...
		this.species = new Species[80];// cambiar a 80
			...
			case 1:
				for (int i = 0; i < species.length; i++) {
					if (species[i] != null && species[specieToEdit - 1] != null) {
						species[specieToEdit - 1].setName(newName);
						result = true;
					}
				}
				break;
			...
			case 3:
				for (int i = 0; i < species.length; i++) {
					if (species[i] != null && species[specieToEdit - 1] != null) {
						...
						if (species[specieToEdit - 1].getType() == SpeciesType.LAND_FLORA) {
							species[specieToEdit - 1].setType(SpeciesType.AQUATIC_FLORA);
						} else if (species[specieToEdit - 1].getType() == SpeciesType.AQUATIC_FLORA) {
							species[specieToEdit - 1].setType(SpeciesType.LAND_FLORA);
